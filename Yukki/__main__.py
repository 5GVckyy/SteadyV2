...
async def initiate_bot():
    ...
    console.print(
        "[bold green]Congrats!! Kay Music Bot has started successfully!\n"
    )
    try:
    ...
    if STRING6 != "None":
        try:
            await ASS_CLI_6.send_message(
                LOG_GROUP_ID,
                "<b>Congrats!! Assistant Client 6 has started successfully!</b>",
            )
        except Exception as e:
            print(
                "\nAssistant Account 6 has failed to access the log Channel. Make sure that you have added your Assistant to your log channel and promoted as admin!"
            )
            console.print(f"\n[red]Stopping Bot")
            return
        try:
            await ASS_CLI_6.join_chat("kayzuchannel")
            await ASS_CLI_6.join_chat("KayzuSupport")
        except:
            pass
        console.print(f"├[red] Assistant 6 Started as {ASSNAME6}!")
        console.print(f"├[green] ID :- {ASSID6}!")
    if LOG_SESSION != "None":
        try:
        ...
        try:
            ...
            pass
    console.print(f"└[red] Kay Music Bot Boot Completed.")
    if STRING1 != "None":
    ...
    if STRING6 != "None":
        await pytgcalls6.start()
    await idle()
    console.print(f"\n[red]Stopping Bot")


home_text_pm = f"""ʜᴇʟʟᴏ, ɪ'ᴍ ᴅᴏ𝘀ᴋʏ ꭙ ᴍᴜ𝘀ɪᴄ'ʙᴏᴛ
